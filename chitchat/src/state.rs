...
impl NodeState {
    /// Returns an iterator over keys not marked for deletion and matching the given predicate.
    ...
    pub fn gc_keys_marked_for_deletion(&mut self, max_version_diff: usize) {
        self.key_values.retain(|_, versioned_value| {
            versioned_value.marked_for_deletion
                && versioned_value.version + (max_version_diff as u64) + 5 < self.max_version
        });
    }
...
impl ClusterState {
    ...
    pub(crate) fn apply_delta(&mut self, delta: Delta) {
        // Remove nodes that has to be reset.
        self.node_states
    ...
    pub fn gc_keys_marked_for_deletion(&mut self, marked_for_deletion_key_grace_period: usize, dead_nodes: &HashSet<NodeId>) {
        for (node_id, node_state_map) in &mut self.node_states {
            if dead_nodes.contains(node_id) {
                continue;
            }
            node_state_map.gc_keys_marked_for_deletion(marked_for_deletion_key_grace_period);
        }
    }
    ...
    pub fn compute_delta(
        ...
        for (node_id, node_state_map) in &self.node_states {
            ...
            // Node needs to be reset if its max_version is "too old".
            // Note that there is no need to reset if floor_version = 0.
            if floor_version > 0
                && node_state_map.max_version
...
mod tests {
    ...
    fn test_cluster_state_apply_delta() {
        ...
        // Check node2 state is trashed and only populated with the new key_d.
    ...
    #[test]
    fn test_cluster_state_compute_delta_with_old_node_state() {
        let mut cluster_state = ClusterState::default();
        ...
        digest.add_node(node1.clone(), 1);
        {
            let delta = cluster_state.compute_delta(&digest, MTU, HashSet::new(), 20_000);
        ...
        {
            // Node 1 max_version in digest + grace period (10_000) is inferior to the node1
            // max_version in cluster state. Thus we expect the cluster to compute a
            // delta that will reset node 1.
            let delta = cluster_state.compute_delta(&digest, MTU, HashSet::new(), 10_000);
            let mut expected_delta = Delta::default();
