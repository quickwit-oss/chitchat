...
enum Operation {
    ...
    NodeStateAssert {
        ...
        node_id: NodeId,
        predicate_fn: Box<dyn Fn(&NodeState) -> bool>,
...
impl Simulator {
                ...
                Operation::NodeStateAssert {
                    ...
                    if let Some(timeout) = timeout_opt {
                        ...
                        if let Some(node_state) = chitchat_guard.node_state(&node_id) {
                            let predicate_value = predicate_fn(node_state);
                            if !predicate_value {
    ...
    pub async fn spawn_node(&mut self, node_id: NodeId, peer_seeds: Option<Vec<NodeId>>) {
        ...
        let seed_nodes: Vec<_> = peer_seeds
            .unwrap_or(self.node_handles.keys().cloned().collect::<Vec<NodeId>>())
            .iter()
        ...
        let config = ChitchatConfig {
            ...
            gossip_interval: self.gossip_interval.clone(),
            listen_addr: node_id.gossip_public_address,
            seed_nodes,
            failure_detector_config: FailureDetectorConfig {
                initial_interval: self.gossip_interval.clone() * 10,
                ..Default::default()
            },
...
fn check_equal_key_value(key: &str, expected_value: &str) -> Box<dyn Fn(&NodeState) -> bool> {
    let key_string = key.to_string();
    let expected_value_string = expected_value.to_string();
    Box::new(move |node_state: &NodeState| {
        let versioned_value = node_state
            .get_versioned(&key_string)
            .expect("Key is expected to be present");
        versioned_value.value == expected_value_string
    })
}

fn assert_key_present(key: &str) -> Box<dyn Fn(&NodeState) -> bool> {
    let key_string = key.to_string();
    Box::new(move |node_state: &NodeState| {
        info!(key=%key_string, "assert-key-present");
        node_state.get_versioned(&key_string).is_some()
    })
}

fn assert_key_not_present(key: &str) -> Box<dyn Fn(&NodeState) -> bool> {
    let key_string = key.to_string();
    Box::new(move |node_state: &NodeState| {
        info!(key=%key_string, "assert-key-not-present");
        node_state.get_versioned(&key_string).is_none()
    })
}

fn assert_key_marked_for_deletion(key: &str, marked: bool) -> Box<dyn Fn(&NodeState) -> bool> {
    let key_string = key.to_string();
    Box::new(move |node_state: &NodeState| {
        info!(key=%key_string, marked=marked, "assert-key-marked-for-deletion");
        node_state
            .get_versioned(&key_string)
            .unwrap()
            .marked_for_deletion
...
async fn test_simple_simulation_insert() {
    ...
    let operations = vec![
        ...
        Operation::NodeStateAssert {
            ...
            node_id: node_id_1.clone(),
            predicate_fn: check_equal_key_value("key_a", "0"),
            timeout_opt: Some(Duration::from_millis(200)),
        },
        Operation::NodeStateAssert {
            server_node_id: node_id_1.clone(),
            node_id: node_id_2.clone(),
            predicate_fn: check_equal_key_value("key_b", "1"),
            timeout_opt: None,
        },
    ];
...
async fn test_simple_simulation_with_network_partition() {
    ...
    let operations = vec![
        ...
        Operation::NodeStateAssert {
            server_node_id: node_id_1.clone(),
            node_id: node_id_2.clone(),
            predicate_fn: assert_key_not_present("key_b"),
            timeout_opt: Some(Duration::from_millis(500)),
        },
    ];
    simulator.execute(operations).await;
...
async fn test_marked_for_deletion_gc_with_network_partition() {
    ...
    let operations = vec![
        ...
        Operation::NodeStateAssert {
            server_node_id: node_id_2.clone(),
            node_id: node_id_1.clone(),
            predicate_fn: assert_key_present("key_a"),
        ...
        Operation::NodeStateAssert {
            server_node_id: node_id_2.clone(),
            node_id: node_id_1.clone(),
            predicate_fn: assert_key_marked_for_deletion("key_a", true),
            timeout_opt: Some(Duration::from_millis(300)),
        },
        ...
        Operation::NodeStateAssert {
            server_node_id: node_id_2.clone(),
            node_id: node_id_1.clone(),
            predicate_fn: assert_key_not_present("key_a"),
            timeout_opt: Some(Duration::from_millis(300)),
        },
        Operation::NodeStateAssert {
            server_node_id: node_id_1.clone(),
        ...
        Operation::NodeStateAssert {
            ...
            node_id: node_id_1.clone(),
            predicate_fn: assert_key_marked_for_deletion("key_a", false),
            timeout_opt: None,
        },
...
async fn test_simple_simulation_heavy_insert_delete() {
    ...
    let mut keys_operations = Vec::new();
    for _ in 0..10 {
        ...
        keys_operations.push(Operation::InsertKeysValues {
            node_id: node_id.clone(),
            keys_values,
        });
    }

    for operation in keys_operations {
        simulator.execute(vec![operation]).await;
        tokio::time::sleep(Duration::from_millis(1000)).await;
    }

    tokio::time::sleep(Duration::from_millis(3000)).await;
    for (node_id, keys) in keys_values_inserted_per_node_id.into_iter() {
        ...
        for key in keys {
            ...
            let check_operation = Operation::NodeStateAssert {
                server_node_id,
                node_id: node_id.clone(),
                predicate_fn: assert_key_present(&key),
